...
class FileChunker {
  constructor({ filePath, chunkSize }) {
    external_assert_(typeof chunkSize === 'number', `chunkSize ${chunkSize} is not a number`)
    external_assert_(typeof filePath === 'string', `filePath ${filePath} is not a string`)
    this.filePath = filePath
    this.chunkSize = chunkSize
    this.fileDescriptor = external_fs_.openSync(filePath)
    this.fileSize = external_fs_.statSync(filePath).size
    this.nextChunkNumber = 0
    this.isFinished = false
  }

  getChunk(chunkNumber) {
    external_assert_(typeof chunkNumber === 'number', `chunk number ${chunkNumber} is not a number`)
    const chunk = Buffer.alloc(this.chunkSize)
    const offset = 0
    const length = this.chunkSize
    const position = this.chunkSize * chunkNumber
    const bytesRead = external_fs_.readSync(this.fileDescriptor, chunk, offset, length, position)
...
const dropbox = new cjs/* Dropbox */.d8({ accessToken: accessToken });
